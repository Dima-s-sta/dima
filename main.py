...
def load_data():
    ...
    if os.path.exists(DATA_FILE):
        try:
            ...
            messagebox.showerror("Ошибка", "Не удалось прочитать файл данных. Он может быть поврежден.")
            return []
    return []

def save_data():
    """Сохраняет данные в файл JSON с обработкой ошибок ввода-вывода."""
    try:
        with open(DATA_FILE, 'w') as f:
            json.dump(data, f, indent=4)
    except IOError as e:
        messagebox.showerror("Ошибка записи", f"Не удалось сохранить данные в файл.\n{str(e)}")
...
def add_record():
    ...
    if validate_input(date, temp, desc):
        ...
        data.append(record)
        update_treeview()
        save_data() # Сохраняем после успешного добавления
        clear_inputs()
...
def filter_records():
    ...
    if date_filter:
        try:
            ...
            return update_treeview() # Сбрасываем фильтр
...
if __name__ == '__main__':
    ...
    ttk.Button(filter_frame, text="Применить фильтр", command=filter_records).grid(row=0, column=4, padx=5, pady=5)
     ...
     # Лямбда-функция для сброса фильтра и очистки полей ввода для фильтра
     # `update_treeview()` без аргументов покажет все данные.
     # `command` не принимает аргументы напрямую.
     reset_cmd = lambda: [filter_date_entry.delete(0,'end'), filter_temp_entry.delete(0,'end'), update_treeview()]
     
     ttk.Button(filter_frame, text="Сбросить фильтр", command=reset_cmd).grid(row=0, column=5, padx=5, pady=5)
